...
$$
BEGIN
    RETURN 'test_2 вызван с: ' || p_start_date || ' | ' || p_end_ts || ' | ' || p_prev_date;
END;
$$;
...
$$
DECLARE
    c1 CURSOR FOR SELECT start_date, end_ts, prev_date
                   FROM test_2_params
                   ORDER BY start_date;
...
BEGIN
    FOR r IN c1 DO
        ...
        v_end_ts     := r.end_ts;
        v_prev_date  := r.prev_date;
        
        CALL test_2(:v_start_date, :v_end_ts, :v_prev_date);
        
    END FOR;
    
    RETURN 'Пакетный запуск завершен успешно.';
END;
$$;
